...
module.exports.run = async function ({ api, event, args, Users }) {
  try {
    ...
    if (['amar name ki', 'amr nam ki', 'amar nam ki', 'amr name ki'].some(phrase => dipto.includes(phrase))) {
     ...
     const a = (await axios.get(`${link}?text=${dipto}&senderID=${uid}&font=2`)).data.reply;
...
try{
  if (event.type == "message_reply") {
    ...
    if (isNaN(reply)) {
      const b = (await axios.get(`${await baseApiUrl()}/baby?text=${encodeURIComponent(reply)}&senderID=${event.senderID}&font=2`)).data.reply;
